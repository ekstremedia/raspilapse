...
#### 1. daily_timelapse.py Upload Failed
The daily timelapse script created video and keogram successfully but failed on upload with "Video file not found".

**Root cause:** `find_video_file()` and `find_keogram_file()` were searching only in `/var/www/html/videos` directly, but videos are stored in date-organized subdirectories (`/var/www/html/videos/YYYY/MM/`).

**Solution:** Added `**/` prefix to glob patterns to search recursively.

#### 2. Keogram Shows Overlay Bar
The keogram had a dark band at the top from the semi-transparent overlay background.

**Root cause:** Default 5% crop (108px at 4K) wasn't enough to fully remove the 2-line overlay bar with padding.

**Solution:** Increased crop from 5% to 7% (151px at 4K). Updated defaults in both `create_keogram.py` and `make_timelapse.py`.

### Files Changed
- `src/daily_timelapse.py` - Recursive file search with `**/` glob
- `src/create_keogram.py` - Default crop 5% → 7%
- `src/make_timelapse.py` - Added `crop_top_percent=7.0` to keogram call
- `docs/TIMELAPSE_VIDEO.md` - Added keogram section with crop options
- `docs/DAILY_VIDEO.md` - Added keogram crop note
- `CHANGELOG.md` - Added fixes
