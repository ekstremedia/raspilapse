...
### Test Results
- All 301 tests pass (added regression test)
- Code formatted with black

---

## Bug Fix: Lores Stream Format

### Problem
After implementing the lores stream for brightness measurement, the camera failed with:
```
Test shot failed: lores stream must be YUV
RuntimeError: Failed to acquire camera: Device or resource busy
```

### Root Cause
The lores stream was configured with `format: "RGB888"`, but Picamera2 requires lores streams to use YUV format.

### Fix
Changed lores stream format from RGB888 to YUV420:
```python
# Before (WRONG)
...
Updated brightness calculation to extract Y (luminance) plane from YUV data:
```python
# YUV420: Y plane is first 240 rows (for 320x240)
height = 240
gray = lores_array[:height, :].astype(np.float32)
```

### Regression Test Added
Added `test_lores_stream_format_must_be_yuv` to `tests/test_capture_image.py`:
- Verifies lores stream uses YUV420 format
- Clear error message if RGB888 is used
- Would have caught this bug before deployment
