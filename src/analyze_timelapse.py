...
def find_recent_images(output_dir: Path, hours: int = 24) -> List[Tuple[Path, Path]]:
    ...
    for json_path in output_dir.rglob("*_metadata.json"):
        ...
        if json_path.parent.name != "metadata":  # Skip only if parent dir name is exactly "metadata"
...
def create_graphs(data: Dict, output_dir: Path, config: dict):
    ...
    # Plot lux with log scale
    ax.plot(data['timestamps'], data['lux'], color='tab:orange', linewidth=2.5, marker='o', markersize=4, label='Measured Light')
    ax.set_xlabel('Time', fontsize=14, fontweight='bold')
    ax.set_ylabel('Light Level (lux)', fontsize=14, fontweight='bold')
    ax.set_title('Light Levels Over 24 Hours\n(Lux = unit of illuminance, 1 lux = light from 1 candle at 1 meter)',
                 fontsize=16, fontweight='bold', pad=20)
    ax.set_yscale('log')
    ax.grid(True, alpha=0.3, linestyle='--', which='both')

    # Add reference lines for common light levels (if they're in the data range)
    min_lux = min(data['lux']) if data['lux'] else 1
    max_lux = max(data['lux']) if data['lux'] else 100000

    reference_levels = [
        (100000, 'Direct sunlight', 'red'),
        (10000, 'Full daylight', 'orange'),
        (1000, 'Overcast day', 'yellow'),
        (400, 'Sunrise/Sunset', 'pink'),
        (100, 'Very dark day', 'purple'),
        (10, 'Twilight', 'navy'),
        (1, 'Deep twilight', 'darkblue'),
        (0.1, 'Full moon', 'gray'),
    ]

    for lux_val, label, color in reference_levels:
        if min_lux * 0.5 < lux_val < max_lux * 2:  # Only show if relevant to data range
            ax.axhline(y=lux_val, color=color, linestyle=':', linewidth=1, alpha=0.4)
            ax.text(ax.get_xlim()[1], lux_val, f'  {label} ({lux_val:g})',
                   fontsize=9, va='center', alpha=0.7, color=color)

    # Add day/night threshold lines from config
    if 'adaptive_timelapse' in config:
        thresholds = config['adaptive_timelapse']['light_thresholds']
        ax.axhline(y=thresholds['night'], color='navy', linestyle='--', linewidth=2, alpha=0.8,
                   label=f"Night threshold ({thresholds['night']} lux)")
        ax.axhline(y=thresholds['day'], color='gold', linestyle='--', linewidth=2, alpha=0.8,
                   label=f"Day threshold ({thresholds['day']} lux)")
    ...
    ax.plot(data['timestamps'], data['exposure_time'], color='tab:purple', linewidth=2, marker='o', markersize=3)
    ...
    if 'adaptive_timelapse' in config:
        ...
        ax.axhline(y=max_exp, color='red', linestyle='--', linewidth=1.5, alpha=0.7,
    ...
    ax.plot(data['timestamps'], data['analogue_gain'], color='tab:green', linewidth=2, marker='o', markersize=3)
    ax.set_xlabel('Time', fontsize=12)
    ...
    ax.plot(data['timestamps'], data['sensor_temp'], color='tab:red', linewidth=2, marker='o', markersize=3)
    ax.set_xlabel('Time', fontsize=12)
    ...
    # Color temperature
    ax1.plot(data['timestamps'], data['colour_temp'], color='tab:cyan', linewidth=2, marker='o', markersize=3)
    ...
    ax2.plot(data['timestamps'], data['colour_gains_red'], color='red', linewidth=2, marker='o', markersize=3, label='Red Gain', alpha=0.7)
    ax2.plot(data['timestamps'], data['colour_gains_blue'], color='blue', linewidth=2, marker='s', markersize=3, label='Blue Gain', alpha=0.7)
...
def export_to_excel(data: Dict, output_path: Path, hours: int, config: dict, image_pairs: List[Tuple[Path, Path]]):
    ...
    headers = [
        "Timestamp", "Date", "Time",
        "Lux",
        "Exposure (s)", "Analogue Gain",
        "Sensor Temp (°C)", "Color Temp (K)",
        "Color Gain Red", "Color Gain Blue",
        "Digital Gain", "Image File"
    ]
    ...
    if lux_values:
        ...
        row += 1
        ws_stats.cell(row=row, column=1, value="Min:").fill = PatternFill(start_color="E0E0E0", fill_type="solid")
        ws_stats.cell(row=row, column=2, value=round(min(lux_values), 2))
        row += 1
        ws_stats.cell(row=row, column=1, value="Max:").fill = PatternFill(start_color="E0E0E0", fill_type="solid")
        ws_stats.cell(row=row, column=2, value=round(max(lux_values), 2))
        row += 1
        ws_stats.cell(row=row, column=1, value="Average:").fill = PatternFill(start_color="E0E0E0", fill_type="solid")
        ws_stats.cell(row=row, column=2, value=round(np.mean(lux_values), 2))
        row += 1
        ws_stats.cell(row=row, column=1, value="Median:").fill = PatternFill(start_color="E0E0E0", fill_type="solid")
    ...
    hourly_headers = [
        "Hour", "Avg Lux",
        "Avg Exposure (s)", "Avg Gain",
        "Avg Temp (°C)", "Avg Color Temp (K)",
        "Image Count"
    ]
    ...
    for row_idx, (hour, values) in enumerate(sorted(hourly_data.items()), 2):
        ...
        ws_hourly.cell(row=row_idx, column=2, value=round(np.mean([l for l in values['lux'] if l > 0]) if any(l > 0 for l in values['lux']) else 0, 2))
